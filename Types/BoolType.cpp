...
ADDR BoolType::Create(BlockHandler* blockHandler, const string& name) {
    ...
    if (!name.empty()) {
        ...
        blockHandler->References->AddReference(name, BoolType::TYPE_ID, address);
    }
    return address;
}
...
void BoolType::Overwrite(BlockHandler* blockHandler, ADDR to_overwrite, const LexicalResult& overwrite_with) {
    ...
    ADDR addr = BoolType::Create(blockHandler, "");
